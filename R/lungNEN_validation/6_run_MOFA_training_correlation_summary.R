plot_alignment_QC_summary <- function(
  ...
  # ---- collect per-sample QC CSVs ------------------------------------------

  csv_files <- list.files(
    path       = models_dir,
    pattern    = "_quality_check_metrics\\.csv$",
    recursive  = TRUE,
    full.names = TRUE
  )

  csv_files <- csv_files[
    grepl(paste0(matrices_subdir, "/plots/"), csv_files, fixed = TRUE)
  ]

  if (!length(csv_files))
    stop("No _quality_check_metrics.csv files found under '",
         matrices_subdir, "/plots/' in: ", models_dir)

  message("Found ", length(csv_files), " quality check CSV file(s).")

  metrics <- do.call(rbind, lapply(csv_files, read.csv, check.names = FALSE))
  message("Loaded ", nrow(metrics), " rows total.")

  required_cols <- c("Model", "Sample", "Axis", "Correlation", "RMSE")
  missing_cols  <- setdiff(required_cols, colnames(metrics))
  if (length(missing_cols))
    stop("Missing columns in collected CSVs: ",
  ...
  if (is.null(out_prefix))
    out_prefix <- basename(matrices_subdir)
...
loo_root <- '/data/mesomics/work/mesomics2/lipikal/lipikal-2026/Rpackage/PACMOS/PACMOS/Analysis_010426/lungNEN_LOO'

plot_alignment_QC_summary(
  models_dir      = loo_root,
  matrices_subdir = "train_test_Meth_only",
  out_dir         = file.path(loo_root, "QC_summary"),
  axis_rename = c(
    "Factor1"        = "Factor1",
    "Factor2" = "Factor2",
    "Factor5" = "Factor5"
