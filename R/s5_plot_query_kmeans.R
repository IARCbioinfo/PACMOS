...
#'
#' @param lf_cols         Character vector. LF column names for scatter plots.
...
plot_kmeans_query_sample <- function(
  ...
  if (missing(query_sample) || !nchar(query_sample))
    stop("query_sample is required. Provide the sample ID to highlight.")
  ...
  sample_dirs <- sample_dirs[
    file.exists(
      file.path(sample_dirs, matrices_subdir, "plots",
                paste0(basename(sample_dirs), "_sample_clusters.csv"))
    )
  ]

  if (!length(sample_dirs))
    stop("No valid sample dirs found (missing _sample_clusters.csv). ",
         "Run infer_kmeans_labels() first.")

  # ---- restrict to query_sample --------------------------------------------

  sample_dirs <- sample_dirs[basename(sample_dirs) == query_sample]
  if (!length(sample_dirs))
    stop("query_sample '", query_sample, "' not found in models_dir.")

  # ---- process query_sample folder -----------------------------------------
  ...
  if (!file.exists(si_path))
    stop("  stable_input not found: ", si_path)
  ...
  if (!file.exists(cl_path))
    stop("  sample_clusters not found: ", cl_path)
  ...
  if (!is.null(overlap)) {
    ...
    p_heatmap <- ggplot2::ggplot(
      ...
      ggplot2::labs(
        title = paste0(sample_id, " - K-means (clusters vs. Reeference labels)"),
        x     = "Biological Label",
        y     = "K-means Cluster",
        fill  = "% of cluster"
      ) +
