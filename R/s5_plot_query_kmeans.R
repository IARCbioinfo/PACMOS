...
plot_kmeans_query_sample <- function(
  ...
  for (sdir in sample_dirs) {
    ...
    query_sample <- sample_id
    out_dir   <- file.path(sdir, "outputs")
    ...
    if (!dir.exists(out_dir)) {
      warning("  No outputs/ dir found: ", out_dir, " - skipping.")
      next
    }
    ...
    for (pair in lf_pairs) {
      ...
      p <- ggplot2::ggplot(
        ...
        ggplot2::aes(
          x     = .data[[x_col]],
          y     = .data[[y_col]],
          fill  = .data[["mapped_bio_label"]],
          shape = .data[["kmeans_cluster"]]
        )
      ) +
